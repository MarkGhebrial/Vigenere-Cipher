...
from cipher import VigenereCypher
...
class GUI:
    def __init__ (self, root):
        def encrypt(*args):
            encrypted.set(VigenereCypher(key=key.get()).encryptString(toEncrypt.get()))
            toDecrypt.set(encrypted.get())
            decrypt()

        def decrypt(*args):
            decrypted.set(VigenereCypher(key=key.get()).decryptString(toDecrypt.get()))
